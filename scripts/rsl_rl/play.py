...
def main():
    ...
    rsl_rl_version = _metadata.version("rsl-rl-lib")
    if _parse_version(rsl_rl_version) >= _parse_version("4.0.0"):
        # rsl-rl >= 4.0.0 ships exporters on the runner itself
        runner.export_policy_to_jit(path=export_model_dir, filename="policy.pt")
        runner.export_policy_to_onnx(path=export_model_dir, filename="policy.onnx")
    else:
        # legacy path for older rsl-rl
        try:
